...
class Rerankor(object):
    def __init__(self, k1=20, k2=6, lambda_value=0.3):
        super(Rerankor, self).__init__()
        self.k1 = k1
        self.k2 = k2
        self.lambda_value = lambda_value

    def rerank(self, query_features, gallery_features, savepath=None, save=False, isevaluate=False, dataset=None):

        distmat = re_ranking(query_features, gallery_features,
                                   self.k1, self.k2, self.lambda_value, MemorySave=True, Minibatch=2000)

        if save:
            mkdir_if_missing(savepath)
            if query_features.shape[0] == gallery_features.shape[0]:
                scio.savemat(os.path.join(savepath, 't2t_rerank_distmat.mat'),
                             {'dist': distmat})
            else:
                scio.savemat(os.path.join(savepath, 'q2g_rerank_distmat.mat'),
                             {'dist': distmat})

        if isevaluate:
            assert (dataset is not None), "There is no dataset to input"
            from .evaluator import evaluate_all
            if query_features.shape[0] == gallery_features.shape[0]:
                evaluate_all(distmat, query=dataset.train, gallery=dataset.train)
            else:
                evaluate_all(distmat, query=dataset.query, gallery=dataset.gallery)
